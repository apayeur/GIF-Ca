...
class GIF(ThresholdModel) :
    ...
    def __init__(self, dt=0.1):
        ...
        self.C       = 20.0*self.gl     # pF, capacitance
    ...
    def simulateResponseToExcitatoryConductance(self, gAMPA, gNMDA, baseline_current, V0):

        """
        Simulate the spiking response of the GIF model to an input conductance g (nS) with time step dt.
        V0 indicate the initial condition V(0)=V0.
        The function returns:
        - time     : ms, support for V, eta_sum, V_T, spks
        - V        : mV, membrane potential
        ...
        # Define arrays
        V = np.array(np.zeros(p_T), dtype="double")
        gAMPA = np.array(gAMPA, dtype="double")
        gNMDA = np.array(gNMDA, dtype="double")
        baseline_current = np.array(baseline_current, dtype="double")
        spks = np.array(np.zeros(p_T), dtype="double")
        eta_sum = np.array(np.zeros(p_T + 2 * p_eta_l), dtype="double")
        gamma_sum = np.array(np.zeros(p_T + 2 * p_gamma_l), dtype="double")
        ...
        code = """
                 ...
                 for (int t=0; t<T_ind-1; t++) {
                     ...
                     // INTEGRATE VOLTAGE
                     V[t+1] = V[t] + dt/C*( -gl*(V[t] - El) - ( gAMPA[t] + gNMDA[t]/(1. + exp(-V[t]/16.13)/3.57) )*V[t] - eta_sum[t] + baseline_current[t] );
        ...
        vars = ['p_T', 'p_dt', 'p_gl', 'p_C', 'p_El', 'p_Vr', 'p_Tref', 'p_Vt_star', 'p_DV', 'p_lambda0', 'V', 'gAMPA', 'gNMDA', 'baseline_current',
    ...
    def plotParameters(self) :
        ...
        K_support = np.linspace(0,250.0, 500)
    ...
    def printParameters(self):
        ...
        print "C (nF):\t\t%0.6f"    % (self.C)
        print "gl (nS):\t%0.8f"     % (self.gl)
    ...
    @classmethod
    def compareModels(cls, GIFs, fname, labels=None):
        ...
        colors = plt.cm.Set1( np.linspace(0, 1, len(GIFs) ) )
        ...
        plt.plot([K_support[0], K_support[-1]], [0,0], ls=':', color='black', lw=1, zorder=-1)
        ...
        plt.plot([eta_support[0], eta_support[-1]], [0,0], ls=':', color='black', lw=1, zorder=-1)
    ...
    def plotAverageModel(cls, GIFs, fname=None):
        ...
        plt.xlabel('R (MOhm)')
        str_p_mean = '{0:.0f}'.format(np.mean(p_all))
        plt.text(np.mean(p_all), 1.2, r'$\bar{R}$ = '+str_p_mean, fontdict=font)
        ...
        str_p_mean = '{0:.0f}'.format(np.mean(p_all))
        plt.text(np.mean(p_all), 1.2, r'$\bar{\tau}_m$ = '+str_p_mean, fontdict=font)
        Tools.removeAxis(plt.gca(), ['top', 'left', 'right'])
        plt.yticks([])
        ...
        plt.hist(p_all, histtype='bar', color='red', ec='white', lw=2)
        str_p_mean = '{0:.0f}'.format(np.mean(p_all))
        plt.text(np.mean(p_all), 1.2, r'$\bar{E}_L$ = '+str_p_mean, fontdict=font)
        plt.xlabel(r'$E_L$ (mV)')
        Tools.removeAxis(plt.gca(), ['top', 'left', 'right'])
        plt.yticks([])
        ...
        plt.hist(p_all, histtype='bar', color='red', ec='white', lw=2)
        str_p_mean = '{0:.0f}'.format(np.mean(p_all))
        plt.text(np.mean(p_all)-5., 0.7, r'$\bar{V}_r$ = '+str_p_mean, fontdict=font)
        plt.xlabel(r'$V_r$ (mV)')
        Tools.removeAxis(plt.gca(), ['top', 'left', 'right'])
        plt.yticks([])
        ...
        plt.hist(p_all, histtype='bar', color='red', ec='white', lw=2)
        str_p_mean = '{0:.0f}'.format(np.mean(p_all))
        plt.text(np.mean(p_all), 1.2, r'$\bar{V}_t^*$ = '+str_p_mean, fontdict=font)
        plt.xlabel('$V_t^*$ (mV)')
        Tools.removeAxis(plt.gca(), ['top', 'left', 'right'])
        plt.yticks([])
        ...
        plt.hist(p_all, histtype='bar', color='red', ec='white', lw=2)
        str_p_mean = '{0:.1f}'.format(np.mean(p_all))
        plt.text(np.mean(p_all), 1.2, r'$\widebar{\Delta V}$ = '+str_p_mean, fontdict=font)
