#####################################################
#   Master_GIF_Ca.py
#   Process all neurons and extract GIF-Ca parameters.
#   Output files:
...
def process_all_files_for_GIF_Ca(is_E_Ca_fixed=True):
    ...
    for experiment_folder in data_folders_for_separate_experiments:
        for cell_name in CellNames[experiment_folder]:
            ...
            plt.legend()
            plt.tight_layout()
            plt.savefig(path_results  + cell_name + '_GIF_Ca_' + spec_GIF_Ca + 'simulate.png', format='png')
            plt.close()
            ...
            I_Ca = GIF_Ca_fit.I_Ca_with_Deterministic_forceSpikes(I_training, V_training[0],
