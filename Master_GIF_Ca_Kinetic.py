#####################################################
#   Master_GIF_Ca_Kinetic.py
#   Process all neurons and extract GIF-Ca-Kinetic parameters.
#   Output files:
...
def process_all_files_for_GIF_Ca(is_E_Ca_fixed=True):
    if is_E_Ca_fixed:
    ...
    #CellNames['eighth_set'].remove('DRN159_5HT')  # problematic cell
    CellNames['tenth_set'].remove('DRN656_5HT')  # problematic cell
    CellNames['tenth_set'].remove('DRN657_5HT')  # problematic cell
    ...
    for experiment_folder in data_folders_for_separate_experiments:
        for cell_name in CellNames[experiment_folder]:
            ...
            # Save the model
            GIF_Ca_fit.save(path_results + cell_name + '_GIF_Ca_Kinetic_'+spec_GIF_Ca+'ModelParams' + '.pck')
            ...
            plt.subplot(2,1,1)
            plt.plot(time/1000, V,'--r', lw=0.5, label='GIF-Ca')
            ...
            plt.legend()
            plt.tight_layout()
            plt.savefig(path_results  + cell_name + '_GIF_Ca_Kinetic_' + spec_GIF_Ca + 'simulate.png', format='png')
            plt.close()
            ...
            plt.ylabel('Voltage [mV]')
            plt.legend(loc='best')
            plt.savefig(path_results  + cell_name + '_GIF_Ca_Kinetic_' + spec_GIF_Ca + 'simulateForcedSpikes_Training.png', format='png')
            plt.close(fig)

    output_file = open('../../../Dropbox/Recherches/Raphe/GIF-Ca/Results/' + 'GIF_Ca_Kinetic_'+spec_GIF_Ca+'FitPerformance_FreeTauRef.dat','w')
