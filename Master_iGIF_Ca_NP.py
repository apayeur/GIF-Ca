#####################################################
#   Master_iGIF_Ca_NP.py
#   Process all neurons and extract iGIF-Ca-NP parameters.
#   Output files:
#       (1) CellName_iGIF_Ca_NP_Raster.png : Raster of GIF-Ca model vs experiments
#       (2) CellName_iGIF_Ca_NP_FitPerformance.dat : Md* Epsilon_V(test) PVar for cell CellName
#       (3) iGIF_Ca_NP_FitPerformance.dat : Md* Epsilon_V(test) for all cells
#       (4) CellName_iGIF_Ca_NP_ModelParams.pck : Model parameters and filters
...
def process_all_files_for_iGIF_Ca_NP(is_E_Ca_fixed=True):
    ...
    for experiment_folder in data_folders_for_separate_experiments:
        for cell_name in CellNames[experiment_folder]:
            ...
            iGIF_Ca_NP_fit.gamma.setMetaParameters(length=2000.0, binsize_lb=2.0, binsize_ub=500.0, slope=5.0)
            ...
            theta_tau_all = np.linspace(theta_range_min, theta_range_max,
                                        theta_inf_nbbins)  # tau_theta is the timescale of the threshold-voltage coupling
            likelihoods = iGIF_Ca_NP_fit.fit(experiment, theta_inf_nbbins=theta_inf_nbbins, theta_tau_all=theta_tau_all, DT_beforeSpike=5.0, is_E_Ca_fixed=is_E_Ca_fixed)

            if iGIF_Ca_NP_fit.theta_tau < theta_tau_all[0] + 0.1 * (20. - 10.) / 9.:
                theta_tau_all = np.linspace(theta_range_min - 9., theta_range_max - 10., theta_inf_nbbins)
                likelihoods = iGIF_Ca_NP_fit.fit(experiment, theta_inf_nbbins=theta_inf_nbbins, theta_tau_all=theta_tau_all, DT_beforeSpike=5.0, is_E_Ca_fixed=is_E_Ca_fixed)

            while iGIF_Ca_NP_fit.theta_tau > theta_tau_all[-1] - 0.1 * (20. - 10.) / 9.:
            ...
            iGIF_Ca_NP_fit.save(path_results + cell_name + '_iGIF_Ca_NP_'+spec_GIF_Ca+'ModelParams.pck')
            ...
            for tr in experiment.testset_traces:
                ...
                (time, V_est, eta_sum_est) = iGIF_Ca_NP_fit.simulateDeterministic_forceSpikes(tr.I, tr.V[0], tr.getSpikeTimes())
            ...
            # Compute Md*
            Md_star[cell_name] = prediction.computeMD_Kistler(8.0, iGIF_Ca_NP_fit.dt*2.)
