...
iGIF_Ca_NP_fit.Tref  = GIF_Ca_fit.Tref                 # use the same absolute refractory period as in GIF_Ca_fit
iGIF_Ca_NP_fit.eta   = copy.deepcopy(GIF_Ca_fit.eta)   # use the same basis function as in GIF_Ca_fit for eta (filter coeff will be refitted)
iGIF_Ca_NP_fit.gamma = copy.deepcopy(GIF_Ca_fit.gamma) # use the same basis function as in GIF_Ca_fit for gamma (filter coeff will be refitted)

# Perform the fit
iGIF_Ca_NP_fit.fit(experiment, theta_inf_nbbins=theta_inf_nbbins, theta_tau_all=theta_tau_all, DT_beforeSpike=5.0, is_E_Ca_fixed=is_E_Ca_fixed)
...
#################################################################################################
# FIT iGIF_NP (Mensi et al. 2016 with current-based spike-triggered adaptation)
#################################################################################################
# Note: We use same theta_inf_nbbins and theta_tau_all as for the iGIF_Ca_NP model
...
output_file_md.write(CELL_NAME[3:6] + '\t' + SPECIFICATION[4:] + '\t' + str(R_X))
for pred in Md_all:
    output_file_md.write('\t' + str(pred))
...
iGIF_Ca_NP_fit.printParameters()
iGIF.compareModels([iGIF_Ca_NP_fit, iGIF_NP_fit], labels=['iGIF_Ca_NP', 'iGIF_NP'])
...
#Comparison for training and test sets w/ inactivation
V_training = experiment.trainingset_traces[0].V
I_training = experiment.trainingset_traces[0].I
(time, V_GIF, eta_sum, V_t_GIF, S) = iGIF_NP_fit.simulate(I_training, V_training[0])
(time, V, eta_sum, V_t, S) = iGIF_Ca_NP_fit.simulate(I_training, V_training[0])
fig = plt.figure(figsize=(14,5), facecolor='white')
plt.subplot(2,1,1)
plt.plot(time/1000, V_t_GIF,'-', color='orange', lw=0.5, label='iGIF-NP threshold')
plt.plot(time/1000, V_t,'-', color='purple', lw=0.5, label='iGIF-Ca-NP theshold')
plt.plot(time/1000, V_GIF,'-b', lw=0.5, label='iGIF-NP')
...
V_test = experiment.testset_traces[0].V
I_test = experiment.testset_traces[0].I
(time, V_GIF, eta_sum, V_t, S) = iGIF_NP_fit.simulate(I_test, V_test[0])
(time, V, eta_sum, V_t, S) = iGIF_Ca_NP_fit.simulate(I_test, V_test[0])
plt.subplot(2,1,2)
plt.plot(time/1000, V_GIF,'-b', lw=0.5, label='iGIF-NP')
plt.plot(time/1000, V,'-r', lw=0.5, label='iGIF-Ca-NP')
plt.plot(time/1000, V_test,'black', lw=0.5, label='Data')
...
I_Ca = GIF_Ca_fit.I_Ca_with_Deterministic_forceSpikes(I_training, V_training[0], experiment.trainingset_traces[0].getSpikeTimes())
fig = plt.figure(figsize=(14,5), facecolor='white')
plt.subplot(2,1,1)
