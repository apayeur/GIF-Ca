...
class Trace :
    ...
    def getROI_AfterSpikes(self, lb, ub):

        """
        Return indices of the trace which are in ROI. Inclute all datapoints in [lb, ub].
        lb (lower bound): ms
        ub (upper bound): ms
        """
    ...
    def detectSpikes(self, threshold=0.0, ref=6.0):
    ...
    def detectSpikesWithDerivative(self, threshold=15.0, ref=6.0):

        """
        Detect action potentials by size of derivative.
        To avoid multiple detection of same spike due to noise, use an 'absolute refractory period' ref, in ms.
        Code implemented in C.
        """
        ...
        vars = ['p_T_i', 'p_ref_ind', 'p_threshold', 'p_dt', 'V', 'spike_train']
    ...
    def returnSpikeShapes(self, DT_before=10.0, DT_after=20.0):

        """
        Return all spike shapes using spikes in ROI.
        """
        ...
        for i, s in enumerate(self.spks):
            ...
            if s in ROI_ind:
                ...
                # Avoid using spikes close to boundaries to avoid errors
                if s > DT_before_i and s < (len(self.V) - DT_after_i):
                    AP = self.V[s - DT_before_i: s + DT_after_i]
                    all_spikes.append(AP)
                    if i > 2 :
                        prev_spike.append((s - self.spks[i-3])*self.dt)
                        # Determine spike width
                        maxAP_index = np.argmax(np.gradient(AP,self.dt))
                        Vreference = AP[maxAP_index]
                        ind = np.where(np.logical_and(AP >= Vreference - 2, AP <= Vreference + 2))[0]
                        ind = np.max(ind)
                        spike_width.append((ind - maxAP_index)*self.dt)


        support = np.linspace(-DT_before, DT_after, len(all_spikes[0]))
        spike_nb = len(all_spikes)

        return (support, all_spikes, spike_nb, prev_spike, spike_width)
